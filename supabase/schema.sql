...
-- Im SQL-Editor des Projekts ausführen, danach Storage-Bucket `cell-photos` anlegen (nicht public).
...
create policy "boards deletable by creator or owner"
  on public.boards for delete using (created_by = auth.uid());
...
-- Storage: Bucket cell-photos, private.
-- Objektpfad: {board_id}/{user_id}/{cell_id}.jpg
-- Policies analog: Upload nur eigener user_id-Pfad, Lesen nur als Board-Mitglied
-- und nur eigenes Foto oder wenn Board revealed / reveal_enabled = false.
